...
const Campaigns = () => {
  ...
  const token = useSelector(
    (state: RootState) =>
      state.auth.token ?? state.auth.user?.access_token ?? null,
  );
  ...
  const fetchDetails = async (id: number) => {
    ...
    try {
      ...
      console.log("Error loading campaign details", err);
      toast.error("Error loading campaign details");
    }
  };
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      console.log("Failed to start run", err);
      toast.error("Failed to start run");
    } finally {
      setIsRunning(false);
    }
      ...
      <div className="bg-white p-10 rounded-[14px] border border-slate-300 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <p className="text-slate-500 font-medium tracking-tight max-w-xl">
              Initialize and manage high-volume calling campaigns across the
              network.
            </p>
          </div>
          <button
          ...
          <div className="p-8 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-[14px] bg-brand-primary/10 flex items-center justify-center">
                  <Table size={24} className="text-brand-primary" />
                </div>
                <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tighter">
                    CSV Template{" "}
                    <span className="text-brand-primary">Specification</span>
                  </h2>
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Required columns format for contact uploads
                  </p>
                </div>
              </div>
              {/* <button 
              ...
              <div className="space-y-3">
                {campaigns.map((c) => (
                  ...
                  >
                    <div className="text-left">
                      <p
                        className={`font-black text-sm tracking-tight mb-1 ${selectedId === c.id ? "text-white" : "text-slate-900"}`}
                      >
                        {c.name}
                      </p>
                      <p
                        className={`text-[10px] uppercase font-bold tracking-widest ${selectedId === c.id ? "text-white/50" : "text-slate-400"}`}
                      >
                        Deployed: {new Date(c.created_at).toLocaleDateString()}
                      </p>
                    </div>
            ...
            {selectedId ? (
                  ...
                  {[
                    {
                      label: "Total Numbers",
                      value:
                        stats?.total_contacts ?? details?.total_contacts ?? "0",
                      icon: <Users size={20} />,
                    ...
                    },
                    {
                      ...
                      value: stats?.call_stats.completed_calls ?? "0",
                    ...
                    {
                      label: "Failed Calls",
                      value: stats?.call_stats.total_calls ?? "0",
                    ...
                    <div
                      key={i}
                      className="bg-white rounded-[14px] p-8 border border-slate-300 shadow-sm flex flex-col items-center text-center group hover:border-brand-primary transition-all"
                    >
                      <div
                        className={`w-14 h-14 rounded-[14px] ${stat.bg} ${stat.color} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}
                      >
                        {stat.icon}
                      </div>
                ...
                <div className="bg-white rounded-[14px] p-10 md:p-14 border border-slate-300 shadow-sm relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                    ...
                    <div className="space-y-6">
                      ...
                      <p className="text-xs text-slate-500 font-medium leading-relaxed">
                        Upload your contact array with this campaign node via
                        CSV.
                      </p>
                      <label
                        className={`group relative cursor-pointer w-full h-40 bg-slate-50 border-2 border-dashed rounded-[14px] flex flex-col items-center justify-center transition-all ${isUploading ? "border-brand-primary bg-brand-primary/5" : "border-slate-300 hover:border-brand-primary/50 hover:bg-white"}`}
                      >
                        {isUploading ? (
                          <Loader2
                            className="animate-spin text-brand-primary"
                          ...
                          <div className="flex flex-col items-center">
                            <UploadCloud
                              className="text-slate-300 group-hover:text-brand-primary transition-colors mb-3"
                              size={32}
                            />
                    ...
                    <div className="space-y-6">
                      ...
                      <button
                        onClick={handleRun}
                        disabled={
                          isRunning || (details?.total_contacts ?? 0) === 0
                        }
                        className="group relative w-full flex items-center justify-center gap-3 py-5 bg-slate-900 text-white font-black uppercase tracking-widest text-[11px] rounded-[14px] transition-all hover:bg-slate-800 hover:shadow-2xl hover:shadow-slate-900/20 disabled:opacity-30 active:scale-95 overflow-hidden"
                      >
                        <span className="relative z-10 flex items-center gap-2">
                          {isRunning ? (
                            ...
                            <>
                              Initiate Call <ChevronRight size={16} />
                            </>
                          )}
                        </span>
                        <div className="absolute inset-0 bg-linear-to-r from-brand-primary to-brand-secondary opacity-0 group-hover:opacity-100 transition-opacity" />
                  ...
                  {activeRun && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="mt-12 p-8 bg-slate-50 border border-slate-300 rounded-[14px] space-y-8"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-5">
                          <div className="relative">
                            ...
                            <Play
                              size={14}
                              className="absolute inset-0 m-auto text-brand-primary z-10 opacity-50"
                            />
                          </div>
                          <div>
                            <p className="text-[10px] font-black text-brand-primary uppercase tracking-[0.2em] mb-1">
                              Sequence ID: {activeRun.id}
                            </p>
                            <div className="flex items-center gap-2">
                              <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                              <p className="text-lg font-black text-slate-900 uppercase tracking-tighter leading-none">
                                {activeRun.status}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-2xl font-black text-slate-900 tracking-tighter">
                            {activeRun.processed_contacts}{" "}
                            <span className="text-slate-300">/</span>{" "}
                            {activeRun.total_contacts}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        <div className="h-2.5 w-full bg-slate-200 rounded-full overflow-hidden shadow-inner">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{
                              width: `${(activeRun.processed_contacts / activeRun.total_contacts) * 100}%`,
                            }}
                            className="h-full bg-linear-to-r from-brand-primary to-brand-secondary shadow-[0_0_15px_rgba(14,165,233,0.3)]"
                          />
                        </div>
                        <div className="flex justify-between text-[10px] font-black uppercase tracking-[0.2em]">
                          <span className="text-slate-400">
                            ...
                            <span className="text-slate-900">
                              {activeRun.initiated_calls}
                            </span>
                          </span>
                          <span className="text-slate-400">
                            Offline:{" "}
                            <span className="text-red-500">
                              {activeRun.failed_calls}
                            </span>
                          </span>
                        </div>
                      </div>
              ...
              >
                ...
                <p className="text-slate-500 font-medium max-w-sm leading-relaxed">
                  Select a campaign from the archive to synchronize your
                  campaign command and monitor campaign progress.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        {isCreating && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              ...
              <h2 className="text-3xl font-black text-slate-900 tracking-tighter mb-8 text-center">
                Create Campaign
              </h2>
              <form onSubmit={handleCreate} className="space-y-8">
                <div className="space-y-3">
                ...
                <div className="flex gap-4 pt-4">
                  <button
                    type="button"
                    onClick={() => setIsCreating(false)}
                    className="flex-1 py-4 bg-slate-100 text-slate-500 font-black uppercase tracking-widest text-[10px] rounded-[14px] hover:bg-slate-200 transition-all"
                  >
                    Abort
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-4 bg-slate-900 text-white font-black uppercase tracking-widest text-[10px] rounded-[14px] hover:bg-slate-800 transition-all shadow-xl"
                  >
                    Initialize Campaign
                  </button>
                </div>
